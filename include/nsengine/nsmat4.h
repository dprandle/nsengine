...
template <class T>
NSMat4<T> operator*(const int32 & pLHS, const NSMat4<T> & pRHS);
...
struct NSMat4
{
	...
	NSMat4<T> & setColumn(const uint32 & i, const T & x, const T & y, const T & z, const T & w)
	{
		(*this)[0][i] = x; (*this)[1][i] = y; (*this)[2][i] = z; (*this)[3][i] = w;
		return *this;
	}

	NSMat4<T> & setColumn(const uint32 & i, const NSVec4<T> & col)
	{
