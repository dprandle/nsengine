...
template <class T>
NSMat4<T> operator*(const nsint & pLHS, const NSMat4<T> & pRHS);

template <class T>
NSMat4<T> operator*(const nsfloat & pLHS, const NSMat4<T> & pRHS);

template <class T>
NSMat4<T> operator*(const nsdouble & pLHS, const NSMat4<T> & pRHS);
...
struct NSMat4
{
	...
	NSMat4<T> & rotationFrom(const NSVec4<T> & axisAngle, nsbool rads = false)
	{
	...
	NSMat4<T> & rotationFrom(const NSVec3<T> & euler, typename NSVec3<T>::EulerOrder order , nsbool rads = false)
	{
		T cb, sb, ch, sh, ca, sa;
		if (!rads)
		{
	...
	NSMat4<T> & setColumn(const nsuint & i, const T & x, const T & y, const T & z, const T & w)
	{
		(*this)[0][i] = x; (*this)[1][i] = y; (*this)[2][i] = z; (*this)[3][i] = w;
		return *this;
	}

	NSMat4<T> & setColumn(const nsuint & i, const NSVec4<T> & col)
	{
	...
	nsstring toString(bool newline = true) const
	{
		nsstringstream ss;
		nsstring lc = ";";
	...
	//
	const NSVec4<T> & operator[](const nsuint & pVal) const
	{
		if (pVal > 3)
	...
	//
	NSVec4<T> & operator[](const nsuint & pVal)
	{
		if (pVal > 3)
			throw(std::out_of_range("mat4 index out of range"));
		return data[pVal];
	...
	NSVec4<T> operator()(const nsuint & pVal) const
	{
...
template<class PUPer, class T>
void pup(PUPer & p, NSMat4<T> & m4, const nsstring & varName)
{
